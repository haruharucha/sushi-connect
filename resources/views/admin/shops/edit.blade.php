...
<head>
    ...
    <title>店舗情報の編集 - すしコネクト管理画面</title>
                ...
                <div class="card shadow-sm border-0">
                    <div class="card-body p-5 bg-white rounded">
                        ...
                        <h2 class="card-title text-primary fw-bold mb-4">
                            <i class="bi bi-pencil-square"></i> 店舗情報を編集する
                        </h2>
                        <p class="text-secondary mb-4">店舗の情報を変更する場合は、以下の項目を書き換えてください。</p>
                        ...
                        <form action="{{ route('admin.shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('area') is-invalid @enderror" id="area" name="area" required>
                                    ...
                                    <option value="静岡県" {{ old('area', $shop->area) == '静岡県' ? 'selected' : '' }}>静岡県</option>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="tel" class="form-control @error('phone_number') is-invalid @enderror" id="phone_number" name="phone_number" value="{{ old('phone_number', $shop->phone_number) }}" required>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('style') is-invalid @enderror" id="style" name="style" required>
                                    <option value="">選択してください</option>
                                    <option value="回転寿司" {{ old('style', $shop->style) == '回転寿司' ? 'selected' : '' }}>回転寿司</option>
                                    <option value="回らない寿司" {{ old('style', $shop->style) == '回らない寿司' ? 'selected' : '' }}>回らない寿司（カウンター・職人握り）</option>
                                    <option value="寿司居酒屋" {{ old('style', $shop->style) == '寿司居酒屋' ? 'selected' : '' }}>寿司居酒屋</option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('price_range') is-invalid @enderror" id="price_range" name="price_range" required>
                                    <option value="">選択してください</option>
                                    <option value="1,000円〜2,000円" {{ old('price_range', $shop->price_range) == '1,000円〜2,000円' ? 'selected' : '' }}>1,000円〜2,000円（お気軽に）</option>
                                    <option value="3,000円〜5,000円" {{ old('price_range', $shop->price_range) == '3,000円〜5,000円' ? 'selected' : '' }}>3,000円〜5,000円（標準的）</option>
                                    <option value="5,000円〜10,000円" {{ old('price_range', $shop->price_range) == '5,000円〜10,000円' ? 'selected' : '' }}>5,000円〜10,000円（プチ贅沢）</option>
                                    <option value="10,000円以上" {{ old('price_range', $shop->price_range) == '10,000円以上' ? 'selected' : '' }}>10,000円以上（高級・特別な日に）</option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="5" required>{{ old('description', $shop->description) }}</textarea>
                                <div class="form-text text-muted">※ここに記述した特徴やキーワード（例：マグロ、個室など）が、一般ユーザーの検索対象になります。</div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                @if ($shop->image_path)
                                    <div class="mb-2">
                                        <small class="text-muted d-block mb-1">現在の画像：</small>
                                        <img src="{{ asset('storage/shops/' . $shop->image_path) }}" alt="現在の画像"
                                            class="img-thumbnail"
                                ...
                                <div class="form-text">※画像を変更したい場合のみ、新しい写真を選択してください。変更しない場合は空欄のままでOKです。</div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex align-items-center">
                                    <input type="time" class="form-control @error('opening_start') is-invalid @enderror" id="opening_start" name="opening_start"
                                        value="{{ old('opening_start', $shop->opening_start ? \Carbon\Carbon::parse($shop->opening_start)->format('H:i') : '') }}" required>
                                    <span class="mx-2">～</span>
                                    <input type="time" class="form-control @error('opening_end') is-invalid @enderror" id="opening_end" name="opening_end"
                                        value="{{ old('opening_end', $shop->opening_end ? \Carbon\Carbon::parse($shop->opening_end)->format('H:i') : '') }}" required>
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-primary px-5 shadow-sm">
                                    <i class="bi bi-check-circle"></i> 変更を保存する
                                </button>
                            </div>
